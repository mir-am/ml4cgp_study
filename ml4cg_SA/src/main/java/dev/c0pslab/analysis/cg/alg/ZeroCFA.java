...
/*
    Creates a call graph based on the Zero CFA approach, which is context-insensitive.
 */
public class ZeroCFA {
    private static final Logger LOG = LoggerFactory.getLogger(ZeroCFA.class);
...
//                            programJarFile.getFileName().toString().replace(".jar", "") +  "_0cfa.csv");
...
//            customThreadPool.submit(() -> filteredTrainProgramsJars.entrySet().parallelStream().forEach((programJar) -> {
        for (var programJar : filteredTrainProgramsJars.entrySet()) {
            ...
            final SSAPropagationCallGraphBuilder cgBuilder = Util.makeZeroCFABuilder(Language.JAVA, opts, cache, chaMap);
            Callable<Void> task = () -> {
                final CallGraph cg;
                try {
                    ...
                    IOUtils.writeWalaCGToFile(cg, programOutputCGFile.toString() + "_0cfa");
